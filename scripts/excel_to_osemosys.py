...
def main(input_workbook, output_file, csv_folder):
    """Read the xlsx file
    """
    csv_from_excel(input_workbook, output_file, csv_folder)


def csv_from_excel(input_workbook, output_file, csv_folder):
    ...
    for i in range(len(sheetNames)):
        ...
        os.makedirs(csv_folder, exist_ok=True)  # creates the csv folder

        # Open the sheet name in the xlsx file and write it in csv format
        with open(os.path.join(csv_folder, modifiedSheetNames[i] + '.csv'), 'w', newline='') as your_csv_file:
    ...
    fileOutput = parseCSVFilesAndConvert(modifiedSheetNames, csv_folder)
...
def parseCSVFilesAndConvert(sheetNames, csv_folder):
    result = ''
    for i in range(len(sheetNames)):
        ...
        if (sheetNames[i] in ['STORAGE', 'EMISSION', 'MODE_OF_OPERATION',
            ...
            with open(os.path.join(csv_folder, sheetNames[i] + '.csv'), newline='') as csvfile:
        ...
        # 24 #all the parameters     that have one variable
            ...
            result += '\n[REGION, *, *]:\n'
            result += insert_table(sheetNames[i], csv_folder)
        # 24 #all the parameters that have one variable
        ...
        # 3 #all the parameters that have 2 variables
            ...
            result += 'param ' + sheetNames[i] + ' default 0 := \n'
            result += insert_two_variables(sheetNames, i, csv_folder)
        # 3 #all the parameters that have 2 variables
        ...
        # 8 #all the parameters that do not have variables
            ...
            result += 'param ' + sheetNames[i] + ' default 9999999 : \n'
            result += insert_no_variables(sheetNames, i, csv_folder)
...
def insert_no_variables(sheetNames, i, csv_folder):
    result = ""
    with open(os.path.join(csv_folder, sheetNames[i] + '.csv'), newline='') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)
...
def insert_two_variables(sheetNames, i, csv_folder):
    result = ""
    with open(os.path.join(csv_folder, sheetNames[i] + '.csv'), newline='') as csvfile:
        reader = csv.reader(csvfile)
        newRow = next(reader)
...
def insert_table(name, csv_folder):
    result = ""
    with open(os.path.join(csv_folder, name + '.csv'), newline='') as csvfile:
        reader = csv.reader(csvfile)
        newRow = next(reader)
